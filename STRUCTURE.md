...
2. 渠道意图写到 body 的 `providerOptions.gateway.order` 数组（如 `["vertex"]` / `["bedrock","anthropic"]`），Vercel 按数组顺序尝试，失败自动 fallback。
3. 只对 `anthropic` / `google` namespace 注入 order——OpenAI 模型只有 openai 一家能服务，强加路由会失败；未知 namespace 不动以免破坏自定义模型。
4. 响应里 `provider_metadata.gateway.routing.resolvedProvider` 能看到实际命中哪家（如 `vertexAnthropic`、`bedrock`、`anthropic`）。
...
3. `rewriteThinkingSuffix` — 模型名 `-thinking-{low,mid,high,max}` 后缀剥离 + 注入 thinking/reasoning
4. `transformReasoning` — `reasoning_effort` 字段 → providerOptions.anthropic.thinking 或 reasoning
...
| `/api/settings` | GET/POST | handleSettings（保存重试状态码、最大尝试次数、Hobby 不可用模型规则） |
...
| `/v1/*` `/aws/v1/*` `/vertex/v1/*` `/anthropic/v1/*` | * | handleGatewayProxy |

## 认证

拆成 Web 登录 token 和 API 调用 token：
- `WEB_AUTH_TOKEN` —— 守 `/`（登录页/SPA）、`/api/login`、`/api/*`
- `API_AUTH_TOKEN` / `api_auth_tokens` —— 守 `/v1/*`、`/aws/v1/*`、`/vertex/v1/*`、`/anthropic/v1/*`，运行配置支持多个 API token；可为空，空时代理接口 fail-closed 返回 401，用户可进控制台后生成。
...
**设置 Tab**：收纳重试状态码、最大尝试次数、Hobby 不可用模型规则。Hobby 规则每行一个模型或前缀，保存到 `state.json` 的 `hobby_blocked_models`。
